...
| Précision du sortant et coût/latence sont-ils mesurés ? | Oui : 6/6 (100%) sur le SIREN du sortant (1 cas exclu et documenté, structurellement indécidable) ; coût 0,00 EUR par briefing (`construire_bloc_de_decision()` n'invoque aucun LLM). Latence mesurée le 16-17/09/2026 (revue à la hausse par rapport aux passes précédentes — volume de données nettement supérieur depuis l'import complet réellement abouti, cf. rapport de stage section 20), 5 répétitions par cas, préchauffage exclu du chronométrage : cas ambigu (Pyrénées-Orientales) médiane **63 ms** ; cas riche (Cour des Comptes) médiane **75 ms** ; données insuffisantes (acheteur inconnu) médiane **97 ms** ; cas pauvre qui déclenche réellement l'agent d'expansion médiane **303 ms**. | `python scripts/mesurer_precision_sortant.py` et `python scripts/mesurer_cout_latence_briefing.py` |
