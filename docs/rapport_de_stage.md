...
**Périmètre couvert par ce rapport :** S1 à S8 dans leur intégralité — **BOAMP est désormais exploré et connecté** (recoupement/détection, cf. section 16), **les 3 agents du sujet sont implémentés** (cf. sections 3, 10, 11 et 15), **Pydantic/JSON Schema sont utilisés** pour la validation systématique des sorties (cf. section 16), et **la verbalisation par LLM est ajoutée** (`scripts/verbaliser_llm.py`, cf. section 17 — alternative disponible, pas encore branchée par défaut sur le bloc de décision). Seule Pappers/Infogreffe (santé financière, API payante) reste hors périmètre — documentée en section 8 (Pistes d'extension) et section 17, pas construite : c'est désormais le seul écart réel restant par rapport au sujet. **Migration de machine (section 20)** : le dépôt tourne désormais sur Windows (PostgreSQL portable) ; l'import complet documenté aux sections 16-19 n'avait en réalité jamais abouti sur cette machine avant le 16-17/09/2026, cf. section 20 pour le détail et le correctif.

**Date de rédaction :** 10/08/2026, mis à jour le 18/08/2026 (cf. section 11), le 20/08/2026 (section 13-14), le 31/08/2026 (agent d'enrichissement web, résolution des entrepreneurs individuels, vérification totale du dépôt — cf. section 15), le 01/09/2026 (BOAMP, Pydantic/JSON Schema, import complet avec filtrage CPV en aval, et correctifs post-migration — cf. section 16), le 02/09/2026 (verbalisation par LLM, et deux bugs réels trouvés et corrigés par une nouvelle vérification totale du dépôt — cf. section 17), le 03/09/2026 (un bug réel trouvé et corrigé dans `detecter_sortant.py` — cf. section 18), le 03/09/2026, plus tard le même jour (l'axe « acheteurs comparables » de l'agent d'expansion réellement branché sur les statistiques de prix/concurrents — cf. section 19), et le 16-17/09/2026 (rebuild complet du pipeline sur la nouvelle machine, jamais abouti jusque-là ; optimisation de coût de la résolution d'identité ; dérive gold trouvée et corrigée — cf. section 20). Tous les chiffres de ce rapport ont été obtenus en relançant les commandes citées à leur date respective, sur l'état du dépôt et de la base à ce moment ; chaque chiffre est accompagné de la commande qui permet de le revérifier.
...
**Tech stack vérifiable** (`requirements.txt`) : PostgreSQL (extensions `pg_trgm` et `vector`/pgvector), SQLAlchemy + psycopg2, DuckDB (filtrage du Parquet DECP côté client), `sentence-transformers` (embeddings locaux), pytest. Depuis le 01/09/2026 (cf. section 17), le SDK `anthropic` est une dépendance déclarée pour `scripts/verbaliser_llm.py` (`claude-haiku-4-5`, dégradation gracieuse sans clé) — mais le chemin de production mesuré en section 6 (`construire_bloc_de_decision()`) n'en dépend toujours pas : aucune dépendance LLM sur le chemin par défaut.

**Sources connectées vs sources du sujet** : le sujet (section 3) recense 6 sources — SIRENE, TED, DECP, BOAMP, Pappers/Infogreffe, web ouvert. **État au 10/08/2026 (ci-dessous, historique) :** ce projet en connectait alors 3 (`connectors/sirene.py`, `connectors/ted.py`, `connectors/decp.py`) ; BOAMP et Pappers/Infogreffe n'étaient ni explorés ni implémentés ; le web ouvert était associé à l'agent d'enrichissement web, non construit. **État au 01/09/2026 (cf. section 16) : BOAMP est connecté** (`connectors/boamp.py`) **et le web ouvert l'est aussi** (`connectors/web_ouvert.py`, agent niveau 5) — seule Pappers/Infogreffe (API payante) reste non connectée, cf. section 8 (Pistes d'extension).
...
Le sujet (section 4) prévoit exactement **trois agents**. **Les 3 sont désormais implémentés** (expansion pilotée par la couverture, le 16/08/2026 ; investigation d'identité, le 18/08/2026 ; enrichissement web, le 31/08/2026, cf. section 15), sans passerelle LLM (aucune n'est configurée dans ce projet) : le « jugement » y est une procédure déterministe qui interroge la base et, pour l'investigation d'identité, l'API SIRENE — ce que demande littéralement le sujet, sans génération de texte libre. `scripts/harnais_evaluation.py` (fonction `cas_non_implementes`) est désormais vide : les 5 pièges de la section 8 sont tous couverts.
...
3. **Enrichissement web — implémentée le 31/08/2026** (`scripts/agent_enrichissement_web.py`, `connectors/web_ouvert.py`, cf. section 15). Seul cas où une recherche sur corpus ouvert est justifiée selon le sujet, conçu en dégradation gracieuse (un échec ne dégrade jamais le briefing en dessous de ce que les sources structurées permettent déjà). Le déroulement du sujet (section 6) le rendait explicitement optionnel pour S6 (*« agent web si le temps le permet »*) ; il n'est associé à aucun piège spécifique de la section 8. Dernier recours après échec des niveaux 1-4 de résolution d'identité, corroboration ≥ 2 résultats + reconfirmation SIRENE obligatoire avant tout retour — réseau-dépendant par nature (scrape l'interface HTML de DuckDuckGo, faute d'API de recherche gratuite stable), dégradation gracieuse vérifiée en conditions réelles.

Les trois agents construits montrent qu'un agent au sens du sujet (une décision dans un espace de recherche ouvert, avec réévaluation) ne présuppose pas une passerelle LLM quand l'espace de décision reste énumérable (quel axe essayer, quand s'arrêter) ou ancré dans une source vérifiable (l'historique SIRENE, ou une corroboration croisée d'un corpus ouvert plutôt qu'une confiance aveugle en un seul résultat).
...
| Taux d'affirmations sourcées | 100% | **100% par construction, pas une statistique échantillonnée** (cf. note ci-dessous) — chaque entrée de `faits` dans `fiche_de_faits.py` porte un champ `provenance` non vide, sans exception dans le code (pas de branche produisant une valeur sans provenance) | — (garanti par la structure du code, aucun script ne calcule de taux) |
| Anti-hallucination (aucun chiffre non sourcé dans le texte généré) | 0 chiffre non justifié | **PASS, garanti par construction, pas une statistique échantillonnée** (cf. note ci-dessous) — `5/5` tests unitaires passent (`tests/test_verification_mecanique.py`, dont `test_texte_avec_chiffre_invente_est_rejete`) | `python scripts/harnais_evaluation.py` (+ `pytest tests/test_verification_mecanique.py`) |
...
**Note sur les métriques 1 et 2 (taux d'affirmations sourcées, taux d'hallucination) — nature de la garantie, vérifié le 01/09/2026.** Contrairement aux 4 autres métriques de ce tableau, il n'existe dans ce dépôt **aucun script qui calcule un pourcentage échantillonné** pour ces deux lignes (vérifié par recherche exhaustive dans `scripts/` et `tests/` : aucune fonction ne compte "N briefings sur M sans hallucination"). Ce sont des **garanties architecturales** : le schéma Pydantic `Fait` (`scripts/schemas.py`) rend le champ `provenance` non nullable pour toute entrée de `faits` (donc 100% n'est pas mesuré, il est *rendu impossible à violer* par la validation systématique à chaque construction de fiche) ; et `scripts/verification_mecanique.py` rejette mécaniquement tout texte contenant un nombre, un nom d'entreprise ou une date absents de la fiche de faits, testé par `tests/test_verification_mecanique.py` (5/5 cas passent, y compris le cas explicite d'un chiffre inventé injecté). **Pourquoi cette approche est jugée suffisante plutôt qu'une limite à combler** : le sujet lui-même (section 4, "mécanisme anti-hallucination") demande une garantie architecturale ("publier un chiffre inventé devient structurellement impossible, pas seulement improbable") — une statistique échantillonnée sur N briefings généreraient au mieux "99,9% de conformité mesurée", ce qui serait une garantie *plus faible* que l'invariant structurel réellement en place ici (validation à 100% des appels, pas un échantillon). Documenter "100%" comme s'il s'agissait d'une fréquence mesurée serait donc trompeur dans l'autre sens. **Limite assumée, conformément au vocabulaire prudent demandé par le sujet (section 2)** : ce dépôt n'a pas fait tourner le pipeline complet sur un grand volume de briefings réels variés pour confirmer statistiquement l'absence de cas limite non couvert par les 5 tests unitaires existants ; l'affirmation correcte est donc *"garanti par construction et vérifié par tests unitaires (5/5)"*, jamais *"mesuré à 100% sur un échantillon de briefings"* — cette dernière formulation ne serait pas vraie de ce dépôt.

**Note sur la métrique 6 (coût et latence par briefing) — absence de tout appel LLM, vérifié le 01/09/2026.** Le "0,00 EUR" mesuré n'est pas le résultat d'un appel LLM peu coûteux : c'est le résultat de l'**absence totale de toute passerelle LLM** dans le chemin de génération d'un briefing. Vérifié directement : `requirements.txt` ne référence aucun SDK LLM (pas de `anthropic`, `openai` ou équivalent) ; `scripts/verbaliser.py` est un gabarit texte pur (f-strings sur des valeurs déjà validées, aucun appel réseau, aucune génération) ; `.env`/`.env.example` ne définissent que `DATABASE_URL` et `SIRENE_API_KEY`, aucune clé LLM. Le sujet (section 9, Environnement technique) anticipe pourtant explicitement "Modèle de langage : passerelle interne, budget précisé au démarrage" comme composant de l'architecture. Ce projet fait le choix assumé et documenté ailleurs (section 3, "frontière déterministe/agentique") de ne jamais invoquer de LLM pour la verbalisation elle-même (un gabarit strict suffit à un texte non génératif) — mais cela signifie que la métrique "coût et latence par briefing" mesure ici un pipeline **sans aucun LLM en boucle**, pas le coût d'un appel LLM réel maîtrisé. C'est cohérent avec le sujet (qui autorise ce choix architectural), mais ce n'est pas la même chose que "mesurer le coût d'un appel LLM" — nuance documentée ici pour éviter toute lecture trompeuse du chiffre "0,00 EUR".
...
- **Passerelle LLM** : aucune n'est configurée dans ce projet. Les 3 agents (cf. section 3) montrent que le « jugement » demandé par le sujet reste réalisable sans elle, tant que l'espace de décision reste énumérable ou ancré dans une source vérifiable. Les embeddings (S4) utilisent volontairement un modèle local (`sentence-transformers`) pour ne pas en dépendre ; la verbalisation (S7) reste un gabarit texte strict, pas une génération par LLM.
...
- **Source BOAMP** (sujet, section 3) — **connectée le 31/08/2026** (`connectors/boamp.py`, cf. section 16) : ce n'est donc plus une piste d'extension.
- **Source Pappers/Infogreffe** (sujet, section 3) : sur les 6 sources recensées par le sujet, SIRENE, TED, DECP, BOAMP et le web ouvert sont désormais connectées (section 2 ci-dessus, section 16). Seule Pappers/Infogreffe (santé financière, **API payante** à couverture partielle selon le sujet lui-même) reste non connectée : aucune clé n'est configurée dans ce projet, et construire ce connecteur sans jamais pouvoir le tester contre le service réel irait à l'encontre du principe déjà appliqué à toutes les autres sources — vérifier en conditions réelles avant de documenter un comportement.
- **Pydantic / JSON Schema** (sujet, section 9) — **corrigé le 31/08/2026** (`scripts/schemas.py`, cf. section 16) : ce n'est donc plus une piste d'extension.
- **Tooling suggéré non repris tel quel** (sujet, section 9) : le sujet suggère `pandas` et un parsing XML (lu *« lamp (XML forms) »*, probablement `lxml`) pour l'ingestion. Ce projet utilise `DuckDB` à la place pour DECP (filtrage SQL direct sur le Parquet source, sans charger le fichier en mémoire via un DataFrame) et consomme TED/BOAMP en JSON via leurs API REST respectives (aucun des deux ne nécessite de parser du XML brut) — un choix technique équivalent sur le fond (extraction/filtrage reproductible, sans boucle Python sur des millions de lignes) mais non identique à l'outillage suggéré, non documenté jusqu'à cette passe.
...
## 16. Vérification totale du 01/09/2026 : import complet, BOAMP, Pydantic/JSON Schema, et correctifs post-migration

Deux évolutions ont été livrées plus tard le 31/08/2026, après la rédaction de la section 15 le même jour, et n'y sont donc pas reflétées — cette section les documente pour la première fois dans ce rapport, avec une vérification totale du dépôt entier menée le 01/09/2026.

**1. Passage à l'import complet, filtrage CPV en aval.** Directive reçue en cours de stage : *la base doit importer toutes les données puis nettoyer, jamais une sélection manuelle à l'import.* Bronze et silver (`bronze_decp_marches`, `bronze_ted_notices`, `silver_marches`, `silver_attributions`) ne filtrent plus par CPV — France/3 ans, tous secteurs (~1,18M lignes DECP, ~8 600 avis TED). Le périmètre CPV 72xxxxxx du sujet (section 6) est appliqué **uniquement** dans `scripts/construire_gold_marches.py` (`WHERE code_cpv LIKE '72%'`), jamais à l'import — filtrer avant le chargement aurait écarté définitivement, sans trace, tout marché mal étiqueté à la source (le sujet documente lui-même, section 3, que les codes CPV DECP sont *« saisis approximativement »*). Migration d'index dédiée (`db/migration_005_index_cpv_silver.sql`, index de motif sur `silver_marches.code_cpv`) nécessaire à cette échelle (~40x plus de lignes qu'avant).

**2. Source BOAMP connectée** (`connectors/boamp.py`, sujet section 3 : *« BOAMP, France avis, Moyenne, Recoupement et détection »*). Exploration en conditions réelles (sujet, S1) : BOAMP ne publie aucun SIRET (ni côté titulaire ni côté acheteur, vérifié sur plusieurs avis réels), contrairement à DECP — utilisé donc strictement dans son rôle documenté par le sujet, jamais comme source d'identité ni d'ingestion gold. `scripts/charger_cache_boamp.py` met en cache les avis « Résultat de marché » (CPV72/France/3 ans) ; `scripts/detecter_recoupement_boamp.py` recoupe ce cache contre gold (acheteur normalisé + CPV4 + fenêtre de date) et rapporte les avis sans correspondance détectée, en lecture seule.

**3. Pydantic et JSON Schema** (sujet, section 9 : *« Sorties structurées : Pydantic et JSON Schéma, validation systématique »*, écart documenté aux sections 9 et 15 ci-dessus). `scripts/schemas.py` définit `FicheDeFaits`/`Fait` et `BlocDeDecision`, tous deux validés systématiquement en sortie (`scripts/fiche_de_faits.py`, `scripts/bloc_de_decision.py`) — une couverture hors [0,1], une provenance vide, ou un bloc de plus de 10 lignes (la contrainte du sujet, section 2, devient une validation structurelle) lèvent une erreur immédiate. `python scripts/schemas.py` régénère `schema/fiche_de_faits.schema.json` et `schema/bloc_de_decision.schema.json`, un artefact JSON Schema indépendant du code Python. Testé (`tests/test_schemas.py`, 6 cas).

**4. Vérification totale du 01/09/2026, ré-exécution effective sur la base réelle.** `pytest tests/` a d'abord révélé un échec réel : `test_marches_similaires_par_texte_retourne_un_score` retournait 0 résultat. Cause trouvée : la reconstruction de `marches` par le nouveau `construire_gold_marches.py` (26 903 marchés, contre 26 830 avant) n'avait pas été suivie d'une régénération des embeddings (`scripts/generer_embeddings_marches.py`) — 0/26 903 marchés avaient un `objet_embedding`, ce qui rendait aussi le piège « CPV mal saisi » du harnais silencieusement `SKIP` plutôt que testé. Corrigé par ré-exécution du script (26 875/26 903 marchés couverts, 100% de ceux ayant un objet non vide).

`python scripts/verification_finale_sirene.py` a de même révélé une régression réelle (6/8 au lieu de 8/8) : 12 entreprises apparues avec le nouveau périmètre gold n'avaient été passées ni par `enrichir_entreprises_depuis_sirene.py`/`enrichir_etablissements_depuis_sirene.py` (jointure contre le stock national), ni par `completer_via_api_sirene.py` (catégorisation du résidu) — le piège « concurrent hors France » du harnais était en conséquence silencieusement inactif (0 entreprise `ETRANGER`, contre 5 attendues). Corrigé en rejouant les trois scripts dans l'ordre documenté par le README : 8/8, 5 entreprises `ETRANGER` retrouvées (TVA intracommunautaires étrangères côté TED), 7 `INTROUVABLE_API`.

Deux docstrings obsolètes (`scripts/agent_expansion_couverture.py`, `scripts/agent_investigation_identite.py`) affirmant encore que l'agent d'enrichissement web était *« hors périmètre »* — alors qu'il est implémenté depuis le 31/08/2026 (section 15) — ont été corrigées. Le tableau de volumétrie du README, resté sur les chiffres du pipeline filtré à l'import, a été resynchronisé avec l'état réel de la base (annexe B et sections 2-3 de ce rapport corrigées de la même façon, cf. notes ci-dessus).

**Résultat final, toutes commandes ré-exécutées le 01/09/2026 :**

| Commande | Résultat |
|---|---|
| `pytest tests/` | **67 passed, 2 skipped** (2 skip = aléa réseau DuckDuckGo documenté, niveau 5 de résolution d'identité) |
| `python scripts/harnais_evaluation.py` | **10/10**, plus aucun piège en `SKIP` (le piège « CPV mal saisi » se déclenche à nouveau réellement) |
| `python scripts/verification_finale_sirene.py` | **8/8** (régression de 6/8 trouvée et corrigée durant cette passe) |
| `python scripts/mesurer_precision_resolution.py` | **92%** global (36/39), **100%** hors homonymie (33/33) — inchangé |
| `python scripts/mesurer_precision_sortant.py` | **6/6 (100%)** sur le SIREN du sortant — inchangé |
| `python scripts/mesurer_cout_latence_briefing.py` | 0,00 EUR par briefing, latences du même ordre de grandeur — inchangé |

**Écarts réels restants, désormais au nombre de deux** (confirmés par `grep` sur `connectors/` et `.env.example` le 01/09/2026) :
- **Pappers/Infogreffe non connectée** (sujet, section 3) — API payante, aucune clé configurée ; non construite plutôt que simulée sans jamais être vérifiée contre le service réel (même principe que pour les autres sources de ce projet).
- **Passerelle LLM générale non configurée** (sujet, section 9) — les 3 agents restent déterministes sans elle (cf. section 3) ; la verbalisation (S7) reste un gabarit texte strict.
- **Tooling suggéré par le sujet non repris à l'identique** (section 9 : `pandas`, parsing XML) : `DuckDB` et une consommation JSON directe des API REST (TED, BOAMP) remplissent le même rôle sans les dépendances suggérées — un choix technique équivalent sur le fond, documenté ici pour la première fois (cf. section 8 ci-dessus).

**Conclusion** : même méthode que les sections 9-15 — ré-exécution effective, jamais une relecture déclarative. Cette passe a trouvé et corrigé deux régressions réelles introduites par le changement d'architecture du pipeline (embeddings non régénérés, catégorisation SIRENE incomplète sur le nouveau périmètre), documenté deux évolutions livrées plus tard le 31/08/2026 et absentes de la section 15, et resynchronisé la documentation (README, ce rapport) avec l'état réel du dépôt et de la base. Le dépôt couvre désormais 5 des 6 sources du sujet (SIRENE, TED, DECP, BOAMP, web ouvert) et les 3 agents prévus ; seule Pappers/Infogreffe (API payante) manque encore, un choix documenté, pas un oubli.

## 17. Vérification totale du 02/09/2026 : verbalisation par LLM, et deux bugs réels trouvés et corrigés

Deux évolutions ont été livrées le 01/09/2026 (après-midi/soir), après la vérification de la section 16 menée le matin même, et n'y sont donc pas reflétées — cette section les documente, avec une nouvelle vérification totale du dépôt (relecture ligne à ligne des scripts restants, pas seulement une ré-exécution des commandes déjà connues) menée le 02/09/2026.

**1. Verbalisation par LLM ajoutée (`scripts/verbaliser_llm.py`), comblant un écart déclaré dans les sections précédentes.** Le sujet (section 4, "Mécanisme anti-hallucination" ; section 9, "Modèle de langage : passerelle interne") décrit un cycle génère → vérifie → régénère par un modèle de langage — jusqu'ici absent de ce dépôt (`scripts/verbaliser.py` restait un gabarit texte pur, sans aucun appel LLM, écart documenté sections 3, 8 et 16 ci-dessus). `verbaliser_via_llm()` ajoute ce cycle : SDK Anthropic officiel (`ANTHROPIC_API_KEY`), modèle `claude-haiku-4-5` (le moins cher de la gamme — tâche fermée de reformulation d'une fiche déjà validée, pas de raisonnement ouvert), vérification par `verification_mecanique.py` après chaque génération, régénération signalant explicitement les valeurs rejetées (`MAX_TENTATIVES=3`), puis repli sur le gabarit déterministe de `verbaliser.py` en cas d'échec répété, d'erreur réseau/authentification, de refus du modèle ou d'absence de clé — jamais de crash, jamais un texte non vérifié livré. **Ce n'est pas encore le chemin par défaut du produit** : `scripts/bloc_de_decision.py` (le bloc de décision, sortie principale du sujet section 2) ne construit ses lignes qu'à partir des faits de la fiche, sans jamais appeler `verbaliser.py` ni `verbaliser_llm.py` ; `verbaliser_via_llm()` reste une alternative disponible pour un futur rapport détaillé en prose (le "complément optionnel" du sujet, section 2), pas branchée sur le chemin de production mesuré en section 6. La mesure "coût et latence par briefing = 0,00 EUR" (section 6 ci-dessus) reste donc exacte et inchangée : elle porte sur `construire_bloc_de_decision()`, qui n'invoque toujours aucun LLM, que `verbaliser_via_llm()` existe par ailleurs ou non.

**2. Deux bugs réels trouvés par une relecture indépendante du dépôt entier (`scripts/`, `connectors/`, `db/`), le 02/09/2026** — même méthode que les sections 10, 11 et 16 : lecture ligne à ligne du code, pas une confiance dans les tests déjà verts, en cherchant spécifiquement des cas où le code contredit sa propre documentation ou le texte du sujet.

- **Bug A (anti-hallucination incomplète, `scripts/verification_mecanique.py`)** : `verifier_texte()` ne contrôlait mécaniquement que les **nombres** du texte généré contre la fiche de faits — jamais les **noms d'entreprise**, alors que le sujet (section 4) et le docstring même de `verbaliser_llm.py` promettent les deux ("tout nombre, tout nom d'entreprise et toute date du texte figurent bien dans la fiche de faits ; sinon, le texte est rejeté et régénéré"). Concrètement, avant correction, un LLM aurait pu inventer un nom de concurrent plausible (ex. "GLOBEX CORPORATION") sans jamais être rejeté par la porte de vérification — la garantie centrale du produit ("publier un chiffre inventé devient structurellement impossible") ne couvrait en réalité que la moitié de ce qu'elle prétend couvrir. **Corrigé** : `extraire_noms_candidats()` extrait les suites de mots tout en majuscules du texte (c'est ainsi qu'une dénomination SIRENE apparaît dans ce gabarit, jamais un mot du gabarit lui-même qui n'a que sa première lettre en majuscule) et les compare à l'ensemble des valeurs autorisées, incluant désormais les noms décomposés depuis les chaînes composites de `fiche_de_faits.py` (ex. "RSM FRANCE (1/11 attribution(s))" autorise aussi "RSM FRANCE" seul). Deux tests de régression ajoutés (`tests/test_verification_mecanique.py::test_texte_avec_nom_entreprise_invente_est_rejete`, `tests/test_verbaliser_llm.py::test_texte_avec_concurrent_invente_declenche_bien_une_regeneration`).
- **Bug B (crash réel, critique, `scripts/fiche_de_faits.py::_valider`)** : `FicheDeFaits(**fiche).model_dump(mode="json", exclude_none=True)` supprimait la clé `"valeur"` de **tout** `Fait` dont la valeur est légitimement `None` (ex. `fourchette_prix_min`/`max` sans aucun montant publié sur la famille) — pas seulement des deux champs racine optionnels (`raison`, `marches_support`) que le docstring visait réellement. `scripts/bloc_de_decision.py` accède à `valeurs["fourchette_prix_min"]["valeur"]` sans `.get()` : la clé manquante levait un `KeyError`, plantant `construire_bloc_de_decision()` — le point d'entrée principal du produit — sur **tout** acheteur/CPV réel sans aucun montant publié. **Vérifié en direct sur la base réelle** : `construire_bloc_de_decision("26750004902888", "72250000", "TEST")` (4 marchés, 0 montant) plantait avant correction, retourne après correction un bloc de décision valide (couverture prix affichée honnêtement à 0%). Le test existant censé couvrir ce cas (`tests/test_bloc_de_decision.py::test_bloc_de_decision_gere_famille_sans_aucun_montant_publie`) ne l'avait pas détecté : il cible un couple acheteur/CPV dont la famille a depuis été élargie par l'agent d'expansion (section 3) et retrouve désormais un montant sur la base actuelle — un test devenu non représentatif du cas qu'il visait, silencieusement, au fil de l'évolution des données. **Corrigé** : `_valider()` n'applique plus `exclude_none` qu'aux deux champs racine réellement optionnels, jamais aux faits imbriqués. Deux tests de régression ajoutés dans un nouveau fichier `tests/test_fiche_de_faits.py`, dont un indépendant de l'état de la base (construit à la main, comme dans `test_verification_mecanique.py`) pour ne pas reproduire le même piège de dérive silencieuse.
- **Bug C (couverture gonflée, mineur, `scripts/fiche_de_faits.py`)** : le fait `duree_restante_mois` gardait la couverture pleine du sortant (`score`) même quand sa valeur est `None` — le cas de **tout** marché TED, qui ne publie jamais ce champ (documenté au README). Comme `couverture_globale` est une moyenne simple des couvertures de chaque fait, un fait sans valeur réelle gonflait artificiellement la couverture globale affichée, sans qu'aucune section du bloc de décision ne le laisse deviner — contraire au principe déjà appliqué ailleurs dans ce fichier (`couverture_expiration`, `couverture_prix`, `couverture_concurrents` dégradent tous correctement à 0 dans le même genre de cas). **Corrigé** : même règle appliquée à `duree_restante_mois` (couverture nulle si valeur absente). Test de régression ajouté (`tests/test_fiche_de_faits.py::test_duree_restante_mois_absente_degrade_sa_propre_couverture`).

**Résultat, toutes commandes ré-exécutées le 02/09/2026 :**

| Commande | Résultat |
|---|---|
| `pytest tests/` | **81 passed, 4 skipped** (85 tests collectés — 2 skip = aléa réseau DuckDuckGo, niveau 5 de résolution d'identité ; 2 skip = `ANTHROPIC_API_KEY` non configurée, tests réels de `verbaliser_llm.py`) |
| `python scripts/harnais_evaluation.py` | **10/10**, tous les pièges du sujet (section 8) toujours couverts après les correctifs |
| `python scripts/verification_finale_sirene.py` | **8/8**, inchangé |

**Écarts réels restants, désormais un seul** (confirmé par `grep` sur `connectors/` et `.env.example`) :
- **Pappers/Infogreffe non connectée** (sujet, section 3) — API payante, aucune clé configurée ; non construite plutôt que simulée sans jamais être vérifiée contre le service réel (même principe que pour les autres sources de ce projet). La passerelle LLM (section 9 du sujet), seul autre écart cité aux sections précédentes, est désormais partiellement comblée par `verbaliser_via_llm()` ci-dessus — pas encore branchée par défaut, mais plus absente du dépôt.

**Conclusion** : même méthode que les sections 10, 11, 15 et 16 — lecture ligne à ligne du code, pas une confiance dans une suite de tests déjà verte. Le bug B (crash) est le plus sérieux trouvé depuis le bug critique de la section 10 : comme lui, il touche le point d'entrée principal du produit sur un cas non hypothétique (vérifié en base réelle), et comme lui, il était resté invisible parce que le test censé le couvrir ciblait un cas dont l'état réel avait changé sous lui — une leçon répétée de ce dépôt (cf. aussi section 16, embeddings non régénérés) : un test qui interroge la base réelle plutôt qu'une fixture construite à la main peut cesser silencieusement de tester ce qu'il prétend tester à mesure que les données évoluent.

## 18. Vérification totale du 03/09/2026 : un bug réel supplémentaire trouvé et corrigé dans `detecter_sortant.py`

Même méthode que les sections 10, 11, 15, 16 et 17 : relecture ligne à ligne du code du chemin critique restant (ici `scripts/detecter_sortant.py`, pas relu en détail depuis la section 14), en cherchant spécifiquement des incohérences entre deux calculs censés porter sur le même marché — le type de bug déjà trouvé trois fois dans ce dépôt (sections 11, 14, 17).

**Bug trouvé (`detecter_sortant()`, gestion des égalités de `date_notification` entre marchés distincts)** : `marche_actuel = resultats[0]` (la ligne SQL la plus récente, tie-break `siren_titulaire ASC` explicite depuis le correctif de la section 14) et `derniere_entree = chaine[-1]` (le dernier élément d'une liste triée par `date_notification` croissante, dédupliquée par `uid`, tri Python stable) sont censés décrire **le même marché** — celui du sortant retourné. Mais leurs tie-breaks respectifs, en cas d'égalité de `date_notification` entre plusieurs `uid` **distincts**, ne sont pas garantis identiques : `resultats[0]` retient le SIREN titulaire le plus petit parmi les ex-aequo, tandis que `chaine[-1]` retient, par stabilité du tri, celui apparu en dernier dans l'ordre d'insertion — deux critères sans rapport l'un avec l'autre.

**Vérifié en base réelle, pas hypothétique** : une requête sur l'ensemble du périmètre gold trouve des dizaines de couples (acheteur, CPV) où la date de notification la plus récente est partagée par plusieurs marchés distincts (jusqu'à 58 `uid` à égalité le même jour sur un cas réel). Cas reproduit avec un impact visible et daté : SIRET `25770538400044`/CPV `72212900` — `marche_actuel` (titulaire retourné comme sortant, ARTAL TECHNOLOGIES) a une durée réelle de 8 mois, mais `chaine[-1]`, avant correction, pointait vers un **autre** marché tié à la même date de notification (2023-09-13) avec une durée réelle de 9 mois. Conséquence : `date_expiration_estimee` (2024-06-13, calculée sur 9 mois) et `duree_source` affichés pour le sortant retourné décrivaient en réalité un marché différent — une échéance fausse d'un mois, sans qu'aucun test existant ne le détecte (aucun des tests de `tests/test_detecter_sortant.py` ne couvrait un cas à égalité de date entre `uid` distincts, seulement le cas déjà corrigé en section 14 : plusieurs titulaires d'un **même** `uid`).

**Corrigé** : `derniere_entree` est désormais recherché explicitement par correspondance d'`uid` avec `marche_actuel` (`next(c for c in chaine if c["uid"] == marche_actuel["uid"])`), jamais un "dernier élément" de liste dont l'identité n'est garantie que lorsqu'aucune égalité de date n'existe. Revérifié sur le cas réel : `date_expiration_estimee` passe de `2024-06-13` (faux) à `2024-05-13` (exact, 8 mois depuis le 2023-09-13 du marché réellement retourné). Test de régression ajouté (`tests/test_detecter_sortant.py::test_echeance_estimee_provient_bien_du_marche_du_sortant_retourne`), construit sur ce cas réel plutôt qu'un exemple fabriqué.

**Revérification complète après correctif** :

| Vérification | Résultat |
|---|---|
| `pytest tests/` | **82 passed, 4 skipped** (81 + 1 test de régression ajouté) |
| `python scripts/harnais_evaluation.py` | **10/10** (inchangé) |
| `python scripts/mesurer_precision_sortant.py` | **6/6 (100%)** sur le SIREN du sortant (inchangé — le bug touchait l'échéance estimée, jamais l'identité du sortant lui-même, donc cette métrique n'était pas affectée) |
| `python scripts/verification_finale_sirene.py` | **8/8** (inchangé) |
| `python scripts/mesurer_precision_resolution.py` | **92%** global (inchangé) |

**Conclusion** : quatrième occurrence dans ce dépôt (après les sections 11, 14 et 17) du même type de défaut — deux valeurs censées décrire le même enregistrement, calculées par deux chemins dont les tie-breaks respectifs ne coïncident pas nécessairement dès qu'une égalité de tri survient. La métrique de précision du sortant (section 8 du sujet, SIREN uniquement) restait vraie malgré ce bug, ce qui l'a rendu invisible à toute vérification par les métriques déjà en place — trouvé uniquement par confrontation directe des deux valeurs sur un cas réel, pas par une régression de test ou de mesure. Corrigé, testé sur données réelles, documenté ici plutôt que silencieusement révisé.

## 19. Vérification totale du 03/09/2026 (suite) : l'axe « acheteurs comparables » de l'agent d'expansion ne servait à rien

Même journée que la section 18, poursuite de la relecture ligne à ligne sur les fichiers du pipeline bronze/silver/gold puis sur l'agent d'expansion (`scripts/agent_expansion_couverture.py`) — cette fois pas un plantage ni une incohérence de tie-break, mais un écart entre ce que le code **prétend** faire (dans son propre docstring, dans le README et dans ce rapport) et ce qu'il fait **réellement**.

**Constat** : le sujet (section 4) prévoit quatre axes d'expansion — « acheteurs comparables, périmètre géographique, CPV parent, fenêtre temporelle ». Les axes « acheteurs comparables » et « périmètre géographique » calculaient bien une liste d'acheteurs comparables (`_acheteurs_meme_naf`, même NAF, département puis national) et la traçaient dans `expansions_appliquees` (visible dans le fait `elargissement_applique` du bloc de décision). Mais cette liste n'était **jamais utilisée** ensuite : `scripts/fiche_de_faits.py` ne lisait jamais `expansion["acheteurs_comparables"]`, et `resultat["historique"]` (qui alimente `concurrents_observes` et `fourchette_prix_min/max`) provenait exclusivement de `detecter_sortant()` sur le seul acheteur d'origine. Le docstring du module et la documentation (README, ce rapport, section 3 et 8) affirmaient pourtant explicitement que cet axe servait à « élargir les statistiques de prix/concurrents » — une affirmation fausse en l'état, vérifiée par recherche exhaustive (`grep acheteurs_comparables scripts/ tests/`) : aucun appelant ne lisait ce champ, et aucun test n'exerçait son effet réel sur les chiffres affichés.

**Corrigé** (choix soumis et validé avant implémentation, vu l'ampleur du changement par rapport aux correctifs d'une ligne des sections précédentes) :
- `scripts/agent_expansion_couverture.py` : nouvelle fonction `_historique_acheteurs_comparables()` qui récupère l'historique (dénomination, montant, `etat_administratif`) des marchés des acheteurs comparables sur le même CPV effectif (celui réellement utilisé par `resultat`, y compris après un éventuel élargissement CPV parent par l'axe 1) — retournée sous `historique_elargi`, sans aucun champ `uid`/`date_notification` (volontairement : cette liste ne doit pouvoir influencer que les statistiques de prix/concurrents, jamais une logique de sortant qui la lirait par erreur — « jamais le sortant », déjà la promesse du docstring).
- `scripts/fiche_de_faits.py` : fusionne `historique_elargi` dans `historique` avant de calculer `concurrents_observes`/`fourchette_prix_min/max` — le sortant lui-même (`titulaire_actuel`, `date_expiration_estimee`, `duree_restante_mois`) reste strictement celui de l'acheteur d'origine, figé avant cette fusion. Couverture des deux faits élargis dégradée de moitié (même principe que `couverture_expiration` pour une durée inférée par médiane CPV) : un périmètre plus large, moins spécifique, jamais présenté avec la même certitude qu'une requête directe sur l'acheteur exact.
- **Effet de bord découvert en testant sur un cas réel** (SIRET `05781313100026`/CPV `72413000`) : la liste de concurrents peut désormais atteindre 81 noms distincts (élargissement national sur le NAF de l'acheteur) — contraire au sujet, section 2 : « bloc de décision ... lisible en 30 secondes ». Corrigé par un plafond (`MAX_CONCURRENTS_AFFICHES = 10`, triés par fréquence décroissante — le signal le plus actionnable pour une décision GO/No-Go), le reste compté explicitement (« ... et N autre(s) concurrent(s) observé(s) »), jamais tronqué silencieusement.

**Tests de régression ajoutés** sur ce même cas réel : `tests/test_agent_expansion_couverture.py::test_agent_elargit_reellement_les_statistiques_via_acheteurs_comparables` (vérifie que `historique_elargi` est bien peuplé et ne porte aucun champ pouvant influencer un sortant) ; `tests/test_fiche_de_faits.py::test_acheteurs_comparables_elargissent_reellement_concurrents_et_prix` (vérifie le plafond à 10 + décompte, la couverture dégradée, et que le sortant retourné reste inchangé).

**Revérification complète après correctif** :

| Vérification | Résultat |
|---|---|
| `pytest tests/` | **84 passed, 4 skipped** (82 + 2 tests de régression ajoutés) |
| `python scripts/harnais_evaluation.py` | **10/10** (inchangé) |
| `python scripts/mesurer_precision_sortant.py` | **6/6 (100%)** (inchangé — le correctif ne touche jamais le sortant lui-même) |
| `python scripts/verification_finale_sirene.py` | **8/8** (inchangé) |
| `python scripts/mesurer_cout_latence_briefing.py` | cas pauvre (déclenche l'agent d'expansion) : ~245 ms médiane (vs ~150-200 ms avant, hausse attendue — une requête SQL supplémentaire sur les acheteurs comparables), les 3 autres cas inchangés |

**Conclusion** : contrairement aux sections 10, 11, 14, 17 et 18 (bugs de plantage ou d'incohérence ponctuelle), cette découverte porte sur une **fonctionnalité entière du sujet restée un no-op fonctionnel** depuis son introduction (section 10, 16/08/2026) malgré une documentation qui en affirmait le comportement à chaque passe de vérification suivante (sections 10, 15, 16, 17) sans que personne — ni les vérifications précédentes, ni les tests — ne relise le code jusqu'au bout de la chaîne (agent → fiche de faits → bloc de décision) pour confirmer que la donnée tracée était réellement consommée. Une leçon de plus dans la lignée de ce dépôt (cf. aussi section 15, 16, 18) : une documentation cohérente avec elle-même n'est pas une preuve que le code fait ce qu'elle décrit — seule la lecture de la chaîne complète, jusqu'au dernier consommateur, le confirme.

## 20. Vérification totale du 16-17/09/2026 : le rebuild post-migration n'avait jamais abouti, optimisation de coût, et dérive gold

Cette section documente une nouvelle vérification totale du dépôt, sur une machine différente de celle des sections 1 à 19 (migration Mac → Windows, cf. `.env`, `venv`, et l'installation PostgreSQL portable documentés dans les commandes ci-dessous). Même méthode que les sections 9 à 19 : ré-exécution effective, jamais une relecture du texte.

**État trouvé en entrant dans cette passe** : le dernier commit du dépôt (`b6e587e`, section 19) décrivait un état — import complet, résolution d'identité niveau 4, agents des trois types — que la base réelle sur cette machine ne reflétait pas. `pytest tests/` échouait sur 2 tests, dont un ancré directement dans le principe de la section 16 (*« bronze et silver ne filtrent plus par CPV »*) : `test_silver_marches_contient_bien_du_hors_perimetre_cpv` trouvait `silver_marches` **entièrement** restreinte au périmètre CPV72 (0 ligne hors périmètre), alors que le README l'affirme non filtrée depuis le 31/08/2026. `silver_marches` comptait 26 896 lignes — l'échelle de gold, pas celle documentée pour silver (952 024 lignes). Cause trouvée par lecture des journaux locaux : deux tentatives de rebuild (`transformer_silver_20260907_121837.log`, `transformer_silver_20260909_104208_run2.log`) s'étaient arrêtées **immédiatement**, fichiers de log vides — un plantage au tout premier `print()` accentué du script, la console Windows par défaut n'étant pas en UTF-8. Du code non commité corrigeant déjà ce point (garde `sys.stdout.reconfigure(encoding="utf-8")` sur 11 scripts) et un retry/backoff sur les 429 de l'API TED (pagination complète sans filtre CPV, ~410 pages, 429 constaté en conditions réelles le 03/09/2026 d'après le commentaire déjà présent dans le code) existait dans l'arbre de travail, jamais testé ni commité — repris et vérifié ici plutôt que réécrit.

**1. Rebuild naïf : coût prohibitif, jamais tenté jusqu'au bout avant cette passe.** Une première tentative de relancer `transformer_silver_marches.py` avec le correctif d'encodage a révélé pourquoi les tentatives précédentes n'avaient peut-être même jamais été menées à terme intentionnellement : les niveaux 2/3 de résolution d'identité (`_resoudre_acheteurs_niveaux_2_3`, `_resoudre_titulaires_niveaux_2_3`) s'appliquaient à **tout** le périmètre non filtré. Mesuré par requête directe sur bronze avant d'attendre la fin du rebuild : **~47 000 identifiants distincts** à résoudre côté TED (acheteurs + titulaires confondus) contre ~300 documentés pour le seul périmètre CPV72 (sections 3-4) — l'import complet couvre désormais tous les secteurs, pas seulement les services informatiques. `resoudre_par_similarite()` (`pg_trgm` contre 29,8M lignes) coûtant jusqu'à plusieurs secondes par appel, une projection sur ce volume donnait plusieurs heures — la première tentative a été interrompue après confirmation du problème plutôt que menée à son terme.

**2. Correctif : borner le coût, jamais les données.** Les niveaux 2/3 restent appliqués sur `silver_marches`/`silver_attributions` complètes et non filtrées (aucune ligne perdue, le principe de la section 16 reste intact) — seule l'**étape d'enrichissement coûteuse** est restreinte au périmètre CPV72 (`PREFIXE_CPV_PERIMETRE`, même constante que `construire_gold_marches.py`), puisque `construire_gold_marches.py` exclut de toute façon tout le reste. Volume mesuré après correctif : ~2 000 identifiants distincts. Rebuild complet exécuté en ~12 minutes.

**3. Dérive gold découverte en resynchronisant la volumétrie du README.** En recalculant la volumétrie post-rebuild pour le README, l'arithmétique ne reconciliait pas : silver en périmètre (1 042 553 − 1 014 393 = 28 160) moins les exclusions documentées (391 sans acheteur + 470 doublons = 861) donnait 27 299 marchés attendus, mais `SELECT COUNT(*) FROM marches` en retournait 29 351 — 2 052 de trop. Cause : `construire_gold_marches.py` est **purement additif** (`ON CONFLICT DO NOTHING` partout, jamais de suppression) — correct pour un chargement incrémental normal, mais après un rebuild qui change l'ensemble des lignes valides de silver (nouvelle fenêtre de données, nouvelle résolution d'identité), les anciennes lignes gold qui ne correspondent plus à rien en silver restent en place indéfiniment. **Ce même défaut explique rétroactivement une incohérence jamais remarquée dans les rapports précédents** : le calcul équivalent à la section 16 (952 024 − 927 079 − 30 − 39 = 24 876) ne correspondait déjà pas au chiffre publié de 26 903 marchés — un écart de 2 027 lignes, de la même nature, resté invisible faute d'avoir jamais été recalculé explicitement.

**Correctif appliqué, conformément à la règle de sécurité des suppressions du README** : sauvegarde horodatée (`backup_marches_20260916`, `backup_attributions_20260916` — 2 052 et 2 488 lignes respectivement, comptées avant toute suppression), confirmation explicite demandée et obtenue, puis suppression (`DELETE FROM attributions` avant `DELETE FROM marches`, ordre imposé par la contrainte de clé étrangère). `construire_gold_marches.py` compte désormais ce type de dérive à chaque exécution et affiche un avertissement explicite si elle n'est pas nulle — jamais de suppression automatique, seulement un signal ; le nettoyage reste un geste manuel et supervisé, comme le reste des suppressions de ce dépôt.

**4. Un test de régression devenu obsolète avec les données, pas avec le code.** `tests/test_fiche_de_faits.py::test_acheteurs_comparables_elargissent_reellement_concurrents_et_prix` (ajouté section 19, cas réel SIRET `05781313100026`/CPV `72413000`) échouait après le rebuild : ce couple avait, sur les nouvelles données, acquis assez d'historique propre au CPV à 4 chiffres (3 marchés, au-dessus du seuil `SEUIL_COUVERTURE_SUFFISANTE=2`) pour ne plus déclencher l'élargissement jusqu'au CPV à 2 chiffres — la fusion des acheteurs comparables restait donc correcte, mais sur un pool plus étroit (~4 concurrents au lieu de « des dizaines »). Vérifié par requête directe qu'il ne s'agissait pas d'une régression du mécanisme (la fusion produit toujours le bon compte pour le CPV effectif réellement retenu). Remplacé par un cas actuel équivalent, retrouvé par recherche systématique en base (SIRET `81172841900029`/CPV `72250000`, NAF 37.00Z, un seul marché propre sur toute la famille CPV 722xxxxx, 7 acheteurs comparables nationaux portant 24 concurrents distincts) — même comportement vérifié (plafond à 11 lignes, couverture dégradée, sortant inchangé).

**Revérification complète après les 4 correctifs** :

| Vérification | Avant (état trouvé, non fonctionnel) | Après |
|---|---|---|
| `pytest tests/` | 82 passed, 2 failed, 4 skipped | **84 passed, 4 skipped** |
| `python scripts/harnais_evaluation.py` | — (silver/gold non reconstruits, non pertinent) | **10/10** (inchangé) |
| `python scripts/mesurer_precision_resolution.py` | — | **92%** global (36/39), **100%** hors homonymie — inchangé |
| `python scripts/mesurer_precision_sortant.py` | — | **6/6 (100%)** — inchangé |
| `python scripts/verification_finale_sirene.py` | — | **8/8** — inchangé |
| `silver_marches` (lignes, dont hors périmètre CPV72) | 26 896 (0 hors périmètre — bug) | **1 042 553** (1 014 393 hors périmètre — conforme au principe de la section 16) |
| `marches` (gold) | 29 351 (avant nettoyage de la dérive) | **27 299** (après nettoyage ; réconcilie exactement avec le calcul silver − exclusions) |
| Couverture attribution | — | 24 801/27 299 marchés reliés à un titulaire (**90,9%**) |
...
|---|---|---|---|
| S1 | Cadrage (exploration TED/DECP/BOAMP, schéma unifié, France seule ou France+UE) → note d'exploration et schéma cible | ✅ | TED/DECP/BOAMP explorés et connectés (`connectors/ted.py`, `connectors/decp.py`, `connectors/boamp.py` — exploration documentée en tête de fichier le 31/08/2026, cf. section 16), schéma cible peuplé (`db/schema.sql`) ; France seule tranchée en code (`connectors/ted.py`, filtre `buyer-country=FRA`), pas dans une note d'exploration séparée |
...
| S3 | SIRENE et résolution d'identité, jeu de test annoté → précision de résolution mesurée | ✅ | 92% global / 100% hors homonymie (historique : 87%/97% au 10/08/2026, cf. section 4 et section 15) |
