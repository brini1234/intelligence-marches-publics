...
Le choix de ne pas construire les deux agents restants est défendable comme une **frontière de nature**, pas seulement une limite de temps : la majorité de ce qui est construit (S1 partiel, S2-S5, S7, S8 partiel) reste des transformations reproductibles d'une donnée déjà présente en base vers une autre donnée déterminée sans ambiguïté par des règles fixes. L'investigation d'identité et l'enrichissement web demandent au contraire de peser des indices contradictoires ou incomplets et d'assumer un jugement au-delà d'un seuil fixe — exactement la distinction que `scripts/harnais_evaluation.py` matérialise en listant le piège qui en dépend comme `NON IMPLÉMENTÉ` plutôt que de le simuler. L'agent d'expansion, lui, montre qu'un agent au sens du sujet (une décision dans un espace de recherche ouvert, avec réévaluation) ne présuppose pas une passerelle LLM quand l'espace de décision reste énumérable (quel axe essayer, quand s'arrêter).
...
| Suite de tests | — | **38/38 passed** (33/33 au 10/08 + 5 tests ajoutés le 16/08 pour `scripts/agent_expansion_couverture.py`, cf. section 10) | `pytest tests/` |
...
- **S6 — Agents** (investigation d'identité, expansion pilotée par la couverture, enrichissement web optionnel) : les trois agents justifiés en section 3. L'agent d'expansion pilotée par la couverture est implémenté depuis le 16/08/2026 (cf. section 10) ; investigation d'identité et enrichissement web restent hors périmètre. Le seul piège de la section 8 qui en dépend encore (changement de raison sociale) reste visible comme `NON IMPLÉMENTÉ` dans `scripts/harnais_evaluation.py` plutôt que masqué, pour que l'écart reste mesurable à chaque exécution du harnais.
